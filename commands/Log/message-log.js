...
export const data = {
    ...
    async execute(interaction) {
        ...
        const menuName = t("messagelog.menuName", { lng: interaction.locale})
        ...
        .setDescription(t("messagelog.embed", {lng: interaction.locale}))
        .setColor("Purple")
        ...
        collector.on("collect",async (interaction) => {
            ...
            if(interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                ...
                await database.updateOne({ guild_id: interaction.guild.id }, { messagelog_id: interaction.values.join() }, { upsert: true })
                ...
                const { messagelog_id } = await database.findOne({ guild_id: interaction.guild.id })
                const channelLog = interaction.guild.channels.cache.get(messagelog_id)
